---
title: "Práctica Final en Data Driven Security (Master Cybersecurity n.16)"
...
A continuación, realizaremos un análisis exploratorio de los conjuntos de datos entregados en la prática para comprender su estructura y contenido.  
...
El conjunto de datos "Dataset Muestra" contiene las mismas columnas que el conjunto de datos "Dataset Global" y Cada fila representa un incidente y tampoco contiene muestras vacías, luego se podrá utilizará para nuestro estudio.  
...
>Con el anterior estudio se concluye que el **Dataset Muestra** es ya el Dataset Global filtrado y por ello no es posible poblar el Dataset Muestra con mas muestras para éste trabajo. Para conseguirlo se debería probar a descargar nuevas muestras desde el repositorio:   
>
>- Source data [KDD Cup 1999 Data Data Set](https://archive.ics.uci.edu/ml/datasets/kdd+cup+1999+data), pero se aparcará para abordarlo en una prática con mas tiempo de entrega  
>
...
Sección de generación de los diferentes modelos de subset por conjuntos preseleccionado de variables.

#### Conjunto 1 por criterio Enunciado, en el enunciado de la práctica se han seleccionado las siguientes variables:  {-}
...
#### Conjunto 2 por criterio PDF ejmplo, conjuntamente con el enunciado se aporta un ejemplo en PDF de un anterior alumno donde se escogieron este otro conjunto de variables, aunque no conocemos su justificación:  {-}
- **SrcBytes**: Representa la cantidad de bytes de datos enviados desde la fuente al destino.
- **DstBytes**: Representa la cantidad de bytes de datos enviados desde el destino a la fuente.
- **DstHostSameSrvRate**: Representa el porcentaje de conexiones al mismo servicio que la conexión actual en los últimos dos segundos.
...
#### Conjunto 3 por criterio Teórico, por algunas combinaciones de variables que podrían ser consideradas en el contexto de una práctica académica, (según fuente Google + ChatGPT 3):  {-}
...
#### Conjunto 4 por criterio matemático, usando una combinación de selección de las variables finales por criterios puramente estadísticos:  {-}
...
> **Conclusión: nos quedaremos con la propuesta del Dataset con variables escogidas por criterios puramente estadísticos**

### Data Visualization:  
enfrentamos ahora el juego de variables escogido, **'Dataset Muestra Estadístico'**, con varios modelos de entrenamiento para comprobar cual seria mas óptimo para este Dataset  
...
Posteriormente, intentaremos mejorarlo añadiendo alguna nueva variables.  

```{r tunning_RF_model_tests_1, echo=TRUE, cache=TRUE, message=FALSE}
...
> Finalmente Se obtiene una precisión del **`r resultado_final$Accuracy`** equilibrando variables para optimizar ahorro de capacidad de cálculo.
...
```{r tunning_RF_model_tests_2, echo=TRUE, cache=TRUE}
...
kable(A, caption = "Resultado por tipo de Flag") %>%
    kable_styling(bootstrap_options = c("striped", "condensed"), full_width = F)
```
`r round(prop.table(A)*100,1)`

## Enriquecimiento del **Dataset Muestra'**:  

A nivel de ejemplo, se podría mejorar el Dataset ya una vez entrenado el modelo, añadiendo la correspondencia de la variable **'Attack'** con las tácticas del estándard de Mitre ATT&CK y poblar el Dataset con las variables relacionadas con su ID y el ID CWE de la debilidad expuesta, añadiendo más valor al receptor de la información o alerta.  

La clasificación ha sido obtenida de fuentes de búsqueda como Google y ChatGPT 3, en algunos tipos no se ha podido identificar claramente etiquetándolos como **'NotFound'**, estos casos podrían intentarse acabar de identificar usando el ejemplo visto en clase con Node4J, o con bibliotecas mas especializadas. _Se emplaza para futuras ampliaciones de la práctica_
...
# Mostrar la tabla
kable(tabla_relacion, caption = "Resulación Táctica con IDs y CWE") %>%
    kable_styling(bootstrap_options = c("striped", "condensed"), full_width = F)
```

#### Testeo de la eficacia de las nuevas variables.  {-}
_se dispone del código pero no se evalua_
...
kable(tabla_fin, caption = "Resultado por tipo de Attack") %>%
    kable_styling(bootstrap_options = c("striped", "condensed"), full_width = F)
```
# Appendix: Todo el código fuente usado para este informe
