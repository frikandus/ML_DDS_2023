...
indep_var = colnames(training_num) != "Attack"

cl = makePSOCKcluster(5)  #incia paralelismo
...
cart_accuracy <- sum(cart_pred == testing$Attack) / length(cart_pred)

# SVM Support Vector Machine OK
start_time <- Sys.time()
fit.svm <- train(Attack ~ ., data=training, method="svmRadial", metric=metric, trControl=control)
end_time <- Sys.time()
elapsed_time_svm1 <- end_time - start_time
svm1_pred <- predict(fit.svm, testing)
svm1_accuracy <- sum(svm1_pred == testing$Attack) / length(svm1_pred)

results1 <- resamples(list(CART=fit.cart, SVM=fit.svm))
summary(results1)

# Entrenar el modelo de Random Forest
# Medir el tiempo consumido en el cálculo del modelo
start_time <- Sys.time()
rf_model <- randomForest(Attack ~ ., data = training, ntree = 100)
end_time <- Sys.time()
elapsed_time_rf <- end_time - start_time
rf_pred <- predict(rf_model, testing)
rf_accuracy <- sum(rf_pred == testing$Attack) / length(rf_pred)
...
nn_accuracy <- sum(nn_pred == testing$Attack) / length(nn_pred)
...
svm_accuracy <- sum(svm_pred == testing$Attack) / length(svm_pred)
...
nb_accuracy <- sum(nb_pred == testing$Attack) / length(nb_pred)

results <- data.frame(Modelo = c("Random Forest", "Support Vector Machine", "Red Neuronal", "CART", "SVM", "Naive"),
                      Precisión = c(rf_accuracy, svm_accuracy, nn_accuracy, cart_accuracy, svm1_accuracy, nb_accuracy),
                      Tiempo =c(elapsed_time_rf, elapsed_time_svm, elapsed_time_nn, elapsed_time_cart, elapsed_time_svm1, elapsed_time_nb))
print(results)

cm <- confusionMatrix(rf_pred, testing$Attack)
