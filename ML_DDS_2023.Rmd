...
```{r 0d1_study_data, echo=FALSE, cache=TRUE}
# Imprimir la tabla resumen
kable(summary_table2, caption = "Clasificación por Variables cualitativa") %>%
    kable_styling(bootstrap_options = c("striped", "condensed"))
```
```{r 0d2_study_data, echo=FALSE, results='hide', fig.align='center', fig.cap='Variables Cualitativas', cache=TRUE}
...
```{r 1b_study_data, echo=TRUE, results='hide', cache=TRUE}
...
```{r 2a_study_data, echo=FALSE, cache=TRUE}
...
```{r 2b_study_data, echo=TRUE, cache=TRUE}
# Eliminar las muestras duplicadas y conservar solo una muestra de ellas
kdd_data_full_unique <- distinct(data_full, .keep_all = TRUE)
```
```{r 2c_study_data, echo=FALSE, cache=TRUE}
# Calcular el número de muestras duplicadas eliminadas
...
```
```{r 2e_study_data, echo=FALSE, cache=TRUE}
...
```{r 2_2_feature_selection, echo=FALSE, cache=TRUE}
...
```{r 2_1_feature_selection, echo=FALSE, cache=TRUE, fig.align='center', fig.cap='validando variables', out.width='50%', fig.show='hold'}
parte <- 0.1  # 10%, toma mínima de muestras para acelerar el cálculo
...
ok <- as.numeric(round(cm_rf$overall[1], 4)*100)      
ok_ori <- as.numeric(round(cm_ori$overall[1], 4)*100) 
ok_pdf <- as.numeric(round(cm_pdf$overall[1], 4)*100) 
ok_teo <- as.numeric(round(cm_teo$overall[1], 4)*100) 

tabla_resultados <- data.frame(Variable = c("Estadístico", "Enunciado", "Ejemplo_PDF", "Teórico_NET"),
                              Accuracy = c(ok, ok_ori, ok_pdf, ok_teo),
                              Kappa = c(as.numeric(round(cm_rf$overall[2], 4)*100), as.numeric(round(cm_ori$overall[2], 4)*100), as.numeric(round(cm_pdf$overall[2], 4)*100), as.numeric(round(cm_teo$overall[2], 4)*100))
                              )
# Imprimir la tabla resultados
kable(tabla_resultados, caption = "Comparativa por juego de predictores") %>%
    kable_styling(bootstrap_options = c("striped", "condensed"), full_width = F)

plot(output.forest)
heatmap(cm_rf$table)
plot(output.forest_ori)
heatmap(cm_ori$table)
plot(output.forest_pdf)
heatmap(cm_pdf$table)
plot(output.forest_teo)
...
### Compararemos el juego de variables escogido en varios modelos de entranamiento para comprobar cual es mas óptimo  
...
# Red Neuronal  (se desactiva, no acabo de dar con la configuración para que presente resultados positivos)
#start_time <- Sys.time()
...
Se modifica el número de muestras en el dataset de entramiento y se busca ajustar parámetros, incluso añadir nuevas variables.
